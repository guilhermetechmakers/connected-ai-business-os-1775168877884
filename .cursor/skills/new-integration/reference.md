# New Integration Reference

Use this reference with `SKILL.md` to implement new provider integrations with the same standards used for Gmail, Google Drive, Google Calendar, HubSpot, QuickBooks, and Slack.
...
- is auditable (structured logs plus action traces),
- is test-covered before release.

## Where To Implement

- Runtime plus tool catalog: `supabase/functions/_shared/integrations-runtime.ts`
- AI routing plus tool orchestration: `supabase/functions/ai-api/index.ts`
...
- provider-specific safety checks (for example immutable fields or admin-only operations).
...
- Use retry plus backoff for `429`/`5xx`.
